...
> ⚠️ **`JWT_SECRET` — read this before deploying.** `utils/jwtSecret.ts` rejects every known placeholder value (including `change_me_in_production`, the base64 that used to ship in `k8s/secret.yaml`) in any environment, and additionally requires at least 32 characters in production. Outside production it falls back to a random secret written to a local file (`server/.jwt-secret.local`); in production it refuses to boot without a real `JWT_SECRET` (opt out with `ALLOW_EPHEMERAL_JWT_SECRET=true` for a deliberate single-node deployment). `k8s/secret.yaml` no longer ships a value at all — supply the real one from a sealed secret or an external secrets manager, or the pod fails to start. If this manifest was ever applied with the old placeholder value, rotate the secret and treat every existing session as compromised.
