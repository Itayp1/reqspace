...
test.describe('Cross-workspace access control (IDOR regressions)', () => {
  test('a non-member cannot read another workspace\'s collection folders/requests, or write into it', async ({ request }) => {
    ...
    const collRes = await request.post(`${serverOrigin()}/api/workspaces/${owner.workspaceId}/collections`, {
    ...
    const reqRes = await request.post(`${serverOrigin()}/api/collections/${collection._id}/requests`, {
      headers: { cookie: owner.cookie },
    ...
    const listFolders = await request.get(`${serverOrigin()}/api/collections/${collection._id}/folders`, {
      headers: { cookie: outsider.cookie },
    });
    expect(listFolders.status()).toBe(403);

    const listRequests = await request.get(`${serverOrigin()}/api/collections/${collection._id}/requests`, {
      headers: { cookie: outsider.cookie },
    });
    expect(listRequests.status()).toBe(403);

    // Nor read a specific request by id directly.
    const getRequest = await request.get(`${serverOrigin()}/api/requests/${secretRequest._id}`, {
      headers: { cookie: outsider.cookie },
    });
    expect(getRequest.status()).toBe(403);

    // Nor create a folder or request inside the owner's collection.
    const createFolder = await request.post(`${serverOrigin()}/api/collections/${collection._id}/folders`, {
      headers: { cookie: outsider.cookie },
      data: { name: 'Injected Folder' },
    });
    expect(createFolder.status()).toBe(403);

    const createRequest = await request.post(`${serverOrigin()}/api/collections/${collection._id}/requests`, {
      headers: { cookie: outsider.cookie },
    ...
    const ownerRead = await request.get(`${serverOrigin()}/api/requests/${secretRequest._id}`, {
      headers: { cookie: owner.cookie },
    });
  ...
  test('a non-member cannot read another workspace\'s environment by id', async ({ request }) => {
    ...
    const outsiderRead = await request.get(`${serverOrigin()}/api/environments/${env._id}`, {
      headers: { cookie: outsider.cookie },
    });
    expect(outsiderRead.status()).toBe(403);

    const ownerRead = await request.get(`${serverOrigin()}/api/environments/${env._id}`, {
      headers: { cookie: owner.cookie },
    });
    expect(ownerRead.ok()).toBeTruthy();
  });
  ...
  test('a non-member cannot publish a public share link for another workspace\'s collection', async ({ request }) => {
    ...
    const shareAttempt = await request.post(`${serverOrigin()}/api/share/collection/${collection._id}`, {
      headers: { cookie: outsider.cookie },
    });
    expect(shareAttempt.status()).toBe(403);

    // Sanity: the owner (editor+) can.
    const ownerShare = await request.post(`${serverOrigin()}/api/share/collection/${collection._id}`, {
      headers: { cookie: owner.cookie },
    });
  ...
  test('the capture endpoint rejects unauthenticated requests instead of writing into an arbitrary workspace', async ({ request }) => {
    ...
    const before = await request.get(`${serverOrigin()}/api/collections/${owner.workspaceId}/folders`, {
      headers: { cookie: owner.cookie },
    }).catch(() => null);
    void before; // not needed — just documents that the workspace exists before the attempt

    const anonCapture = await request.post(`${serverOrigin()}/api/capture/${owner.workspaceId}/anything`, {
      data: { hello: 'world' },
    });
  ...
  test('an authenticated non-member of the workspace cannot use the capture endpoint either', async ({ request }) => {
    ...
    const captureAttempt = await request.post(`${serverOrigin()}/api/capture/${owner.workspaceId}/anything`, {
      headers: { cookie: outsider.cookie },
      data: { hello: 'world' },
    });
