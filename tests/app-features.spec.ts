...
test.describe('Environment duplicate', () => {
  test('POST /environments/:id/duplicate creates a copy with the same variables', async ({ request }) => {
    ...
    const reg = await request.post(`${serverOrigin()}/api/auth/register`, {
    ...
    const createRes = await request.post(`${serverOrigin()}/api/workspaces/${workspaceId}/environments`, {
      headers: { cookie },
    ...
    const dupRes = await request.post(`${serverOrigin()}/api/environments/${original._id}/duplicate`, {
      headers: { cookie },
    });
