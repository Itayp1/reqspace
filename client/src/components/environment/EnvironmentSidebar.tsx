...
export default function EnvironmentSidebar() {
  ...
  const handleCreateEnv = async () => {
    ...
    const envName = await customPrompt('New Environment', 'New Environment', 'Enter environment name:', 'Create');
    if (!envName) return;
  ...
  const handleDelete = async (id: string) => {
    ...
    const ok = await customConfirm('Delete environment', 'Delete this environment? This cannot be undone.', 'Delete');
    if (!ok) return;
    try {
